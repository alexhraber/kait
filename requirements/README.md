...
The Dockerfile asks the embedded `kait contract` command to resolve a profile
into an ordered manifest list, writes the resulting identity, and installs
only those manifests. The same model drives runtime smoke checks, Buildkite
tags, and the CI/release matrix.
...
`cpu.txt`, `apple.txt`, `nvidia.txt`, `amd.txt`, and `intel.txt` preserve the
vendor-specific PyTorch choices. CPU and Apple are active Linux CPU contracts;
NVIDIA, AMD, and Intel remain explicit accelerator opt-ins until matching
Buildkite hosts are available for hardware smoke proof.
