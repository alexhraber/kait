...
installs only those manifests. Native Apple bundles use the same resolver and
install order. The same model drives runtime smoke checks, Buildkite tags, and
the CI/release matrix.
...
preserve the vendor-specific PyTorch choices. `apple-mps.txt` is used only by
the native macOS arm64 contract and is validated with `torch.backends.mps`.
`apple.txt` remains the manifest name used by already-published Linux Apple CPU
images; new builds must not use it to claim Apple GPU access. NVIDIA, AMD, and
Intel remain explicit accelerator opt-ins until matching Buildkite hosts are
available for hardware smoke proof.
