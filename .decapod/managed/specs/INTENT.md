...
AI/ML runtime: a pinned agent, profile-derived Python toolchains, hardware
execution contracts, immutable identity, and collector-friendly metrics.
Operators pick a workload profile and hardware/runtime selector, then run
ordinary Buildkite jobs in the validated execution surface.
...
- **Version lockstep:** release builds inject the semver into `cmd/kait`'s
  version variable so doctor/smoke/Prometheus match the immutable artifact tag.
...
- [ ] Each supported hardware/profile target is versioned and reproducible from the shared container/native projections, with `slim`/`full` compatibility aliases preserved.
- [ ] Every published target declares its supported runtime platform; GPU surfaces remain limited to platforms supported by their runtime.
- [ ] CI builds and runs `kait smoke` for every active container and native hardware/profile target on a matching host class before the workflow can pass.
